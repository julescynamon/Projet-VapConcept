...
var monDataSet = [20000, 15000, 5000, 15000, 10000, 20000, 15000, 25000, 20000, 30000, 25000, 10000];
...
mois.forEach((el, id) => {
  var minDate = date.getFullYear() + "-" + el + "-01";
  var maxDate = date.getFullYear() + "-" + el + "-31";

  var cdrDuMois = 0;

  // Find lignes by date
  db.find({ $and: [{ date: { $gte: minDate } }, { date: { $lte: maxDate } }] }, function (err, docs) {
    // Calcul du CDR du mois
    docs.forEach((el, id) => {
      if(el != null) {
